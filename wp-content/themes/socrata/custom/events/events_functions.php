...
function custom_events() {
  ...
  <div class="format_text">
	...
	<?php
  	$featuredevent_query = new WP_Query('post_type=tribe_events&tribe_events_cat=featured&orderby=desc&showposts=1');
  	while ($featuredevent_query->have_posts()) : $featuredevent_query->the_post(); ?>
...
<?php
	$upcomingevent_query = new WP_Query();
    $upcomingevent_query->query('post_type=tribe_events&showposts=100');
    if ($upcomingevent_query->post_count == 0) { ?>
    <p>There are no events scheduled at this time.</p>
    <?php }
    else { while ($upcomingevent_query->have_posts()) : $upcomingevent_query->the_post(); ?>
